...
source,face_detector, age_gender,bRuning = menu()
...
def load_face_HAAR():
    face_net = cv.CascadeClassifier('face_haar/haarcascade_frontalface_alt.xml')
    return face_net
    
    
def detect_face_HAAR(img,face_net):
    gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
    # Detect faces
    detections = face_net.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5)
    face_boxes = []
...
def predictAgeGender_ssrnet(faces,gender_net,age_net):
    face_size = 64
...
def load_agender_Caffenet():

    global face_padding_ratio
    face_padding_ratio = 0
...
def predictAgeGender_Caffenet(faces,gender_net,age_net):
...
load_face_dict = {
'HAAR':load_face_HAAR,
'Tensornet':load_face_Tensornet
}

detect_face_dict = {
'HAAR':detect_face_HAAR,
'Tensornet':detect_face_Tensornet
}

load_agender_dict = {
'ssrnet':load_agender_ssrnet,
'Caffenet':load_agender_Caffenet
}

predictAgeGender_dict = {
'ssrnet':predictAgeGender_ssrnet,
'Caffenet':predictAgeGender_Caffenet
}    
   

#generate a new video with the face rectangle
...
def process_image(frame,face_net,gender_net,age_net):
    ...
    detect_face_fun = detect_face_dict.get(face_detector)
        
    predictAgeGender_fun = predictAgeGender_dict.get(age_gender)
        
    # Detect faces
    face_boxes = detect_face_fun(frame_bgr,face_net)
    ...
    if (len(face_boxes) > 0):
        ...
        # Get age and gender
        labels = predictAgeGender_fun(faces,gender_net,age_net)
    ...
    time_end = time.time()
    print ("Process one image Time:",time_end - time_start)
...
def main():
    ...
    if bRuning:
        ...
        time_start = time.time()
        load_face_fun = load_face_dict.get(face_detector)
        face_net = load_face_fun()
        time_end = time.time()
        print ("load face detector Time:",time_end - time_start)
        
        time_start = time.time()
        load_agender_fun = load_agender_dict.get(age_gender)
        gender_net,age_net = load_agender_fun()
        time_end = time.time()
        print ("load agender detector Time:",time_end - time_start)
        ...
        if source == 'images':
            for file_name in os.listdir('images'):
                ...
                process_image(img,face_net,gender_net,age_net)
            ...
            for file_name in os.listdir('videos'):
                ...
                if os.path.isfile(file_name) and file_name.endswith('.mp4'):
                    ...
                    while cap.isOpened():
                        ...
                        if not success:
                            break
                        process_image(frame,face_net,gender_net,age_net)
